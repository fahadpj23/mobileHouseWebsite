...
export const handler: Handler = async (event) => {
  try {
    // Extract seriesId from query parameters
    ...
    // Get products from Firestore filtered by seriesId
    const productsRef = db.collection("products");
    const snapshot = await productsRef.where("seriesId", "==", seriesId).get();
    ...
    console.error("Error in get-products-by-series:", error);
    return {
      statusCode: 500,
