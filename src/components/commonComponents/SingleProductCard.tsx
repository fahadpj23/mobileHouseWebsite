...
const SingleProductCard: FC<props> = ({ product }) => {
  ...
  return (
    <Link to={`/phone/${product?.id}/${encodeURIComponent(product?.name)}`}>
      <div className=" p-3 md:p-6 flex flex-col space-y-1   bg-white relative rounded-md">
        <div className="flex justify-center bg-[#f9f6f6] rounded-md shadow-md p-2">
        ...
        <div className="flex flex-col space-y-1 p-1 ">
          <h1 className="font-semibold  w-full  truncate capitalize text-[12px] ">
            {product?.name}
          </h1>
          ...
          <div className="flex space-x-3 items-center">
            <h1 className=" font-medium text-[12px] tracking-wider border-2 border-green-500 p-1 text-green-500 bg-white rounded-md">
